...
class Penci_Soledad_Dashboard {
	...
	public function update() {
		...
		if ( ! empty( $_POST ) && isset( $_POST['_page'] ) && $_POST['_page'] === 'soledad_custom_fonts'  ) {
			$nonce = isset( $_REQUEST['penci_update_options_check'] ) ? $_REQUEST['penci_update_options_check'] : '';
			if ( ! wp_verify_nonce( $nonce, 'penci_update_options' ) ) {
				die( __( 'Security check', 'soledad' ) ); 
			} else {
	...
	public function custom_fonts() {
		...
		<div class="wrap about-wrap penci-about-wrap">
			<?php include PENCI_SOLEDAD_DIR . '/inc/dashboard/sections/welcome.php'; ?>
			<?php include PENCI_SOLEDAD_DIR . '/inc/dashboard/sections/custom-fonts.php'; ?>
		</div>
		<?php
	}
